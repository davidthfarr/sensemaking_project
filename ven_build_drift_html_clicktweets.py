...
SUM_PATH = Path("data/evaluated/ven/cluster_summaries.parquet")
LABEL_PATH = Path("data/evaluated/ven/cluster_labels.parquet")
...
def build_html_with_click(
    ...
    # Collect PCA bounds from all scatter traces
    xs, ys = [], []
    ...
    fig.update_layout(
        ...
        title="Narrative Drift (click a cluster to reveal AI summary of narrative or hover for cluster representative tweets)",
        xaxis_title="PCA-1",
        yaxis_title="PCA-2",
            ...
            dict(
                ...
                buttons=[
                    dict(
                        label="◀ Prev",
                        method="animate",
                        args=[[], dict(mode="immediate", frame=dict(duration=0, redraw=False), transition=dict(duration=0))],
                    ),
                    dict(
                        label="Next ▶",
                        method="animate",
...
def main():
    ...
    var SUMMARIES = SUMMARIES_PAYLOAD_PLACEHOLDER;
    var WINDOWS = WINDOWS_PAYLOAD_PLACEHOLDER;
    var CURR_IDX = 0;
    var GD = null;
    var LABELS = LABELS_PAYLOAD_PLACEHOLDER;
    ...
    function setTweetTrace(gd, points, titleKey) {
      ...
      Plotly.relayout(gd, { "title.text": "Narrative Drift (selected: " + titleKey + ")" });
    }

    function ensureSidePanel() {
      var existing = document.getElementById("cluster-panel");
    ...
    panel.innerHTML = `
      <div style="display:flex; align-items:center; justify-content:space-between; gap:8px;">
        <div style="font-weight:700; font-size:14px;">Cluster details</div>
        <div style="display:flex; gap:6px;">
          <button id="btn-prev" title="Previous window"
            style="cursor:pointer; border:0; color:#EEE; background:rgba(255,255,255,0.12);
                   padding:4px 8px; border-radius:6px;">◀</button>
          <button id="btn-next" title="Next window"
            style="cursor:pointer; border:0; color:#EEE; background:rgba(255,255,255,0.12);
                   padding:4px 8px; border-radius:6px;">▶</button>
          <button id="cluster-panel-close"
            style="cursor:pointer; border:0; color:#EEE; background:rgba(255,255,255,0.12);
                   padding:4px 8px; border-radius:6px;">Hide</button>
        </div>
      </div>
      <div style="margin-top:10px; opacity:0.85;">
        Click a cluster to see its LLM summary.
      </div>
      <hr style="border:none; border-top:1px solid rgba(255,255,255,0.12); margin:10px 0;" />
      <div id="cluster-panel-body"></div>
    `;
      ...
      document.getElementById("cluster-panel-close").onclick = function () {
    ...
    function setSummaryPanel(key) {
      var label = (LABELS && LABELS[key]) ? LABELS[key] : "Untitled cluster";
      var panel = ensureSidePanel();
      panel.style.display = "block";

      var body = document.getElementById("cluster-panel-body");
      if (!body) return;

      var summary = (SUMMARIES && SUMMARIES[key]) ? SUMMARIES[key] : "(No summary found for this cluster.)";

        body.innerHTML = `
          <div style="font-weight:800; font-size:16px; line-height:1.2; margin-bottom:8px;">
            ${escapeHtml(label)}
          </div>
        
          <div style="margin-bottom:10px; opacity:0.85;">
            <code style="opacity:0.85;">${escapeHtml(key)}</code>
          </div>
        
          <div style="font-weight:700; margin-bottom:6px;">LLM summary</div>
          <div style="line-height:1.35; white-space:pre-wrap;">${escapeHtml(summary)}</div>
        `;
        }
    ...
    function bindWhenReady() {
      ...
      GD = gd;
      moveModebarTopCenter();
      bindTimelineButtonsGlobal();



        // Ensure panel exists so buttons exist
        var panel = ensureSidePanel();
        panel.style.display = "block";
        
        // Bind arrow buttons once
        var prevBtn = document.getElementById("btn-prev");
        var nextBtn = document.getElementById("btn-next");
        
        if (prevBtn && !prevBtn.__bound) {
          prevBtn.__bound = true;
          prevBtn.onclick = function() { prevWindow(GD); };
        }
        if (nextBtn && !nextBtn.__bound) {
          nextBtn.__bound = true;
          nextBtn.onclick = function() { nextWindow(GD); };
        }

      gd.on("plotly_animated", function(e) {
          try {
            var name = (e && e.name) ? e.name : (gd._fullLayout && gd._fullLayout._currentFrame);
            var idx = WINDOWS.indexOf(name);
            if (idx >= 0) CURR_IDX = idx;
          } catch (err) {}
        });
    ...
    function goToIndex(gd, idx) {
      if (!WINDOWS || WINDOWS.length === 0) return;
      if (idx < 0) idx = 0;
      if (idx >= WINDOWS.length) idx = WINDOWS.length - 1;
      CURR_IDX = idx;
    ...
    function nextWindow(gd) { goToIndex(gd, CURR_IDX + 1); }
    function prevWindow(gd) { goToIndex(gd, CURR_IDX - 1); }
    function bindTimelineButtonsGlobal() {
      var tries = 0;
    
      function attempt() {
        tries += 1;
    
        // Find updatemenu buttons anywhere in the document (Plotly may not put them under gd)
        var btns = document.querySelectorAll(".updatemenu-button");
        if (!btns || btns.length === 0) {
          if (tries < 40) return setTimeout(attempt, 100);
          console.warn("No .updatemenu-button found");
          return;
        }
    
        // Bind based on visible label text (more stable than relying on index)
        var boundPrev = false, boundNext = false;
    
        btns.forEach(function(b) {
          if (b.__boundNav) return;
    
          var labelNode = b.querySelector("text");
          var label = labelNode ? labelNode.textContent.trim() : "";
    
          if (label.includes("Prev") || label.includes("◀")) {
            b.__boundNav = true;
        ...
        if (boundPrev || boundNext) {
          console.log("Timeline buttons wired:", {boundPrev, boundNext});
          return;
        }
    
        // If buttons exist but labels weren't found yet, retry
        if (tries < 40) return setTimeout(attempt, 100);
        console.warn("Found buttons but could not match Prev/Next labels");
      }
    
      attempt();
    }
    function moveModebarTopCenter() {
      var tries = 0;
      ...
      function attempt() {
        ...
        if (!mb) {
          if (tries < 40) return setTimeout(attempt, 100);
          console.warn("modebar not found");
          return;
        }
    
        // Make it horizontal
        mb.style.display = "flex";
        mb.style.flexDirection = "row";
        ...
        mb.style.top = "8px";
        mb.style.left = "66%";
        mb.style.transform = "translateX(-50%)";
    
        // Ensure it floats above plot but below overlays if desired
        mb.style.zIndex = "9000";
    
        // Optional visual polish
        mb.style.background = "rgba(0,0,0,0.6)";
        mb.style.borderRadius = "10px";
        mb.style.padding = "4px 6px";
        mb.style.boxShadow = "0 4px 14px rgba(0,0,0,0.35)";
    
        console.log("modebar moved to top center");
      }
  ...
  attempt();
    ...
    # --------------------------------------------------
    # Inject payloads AFTER maps exist
    # --------------------------------------------------
    post_script = post_script.replace(
        "TWEETS_PAYLOAD_PLACEHOLDER",
        json.dumps(tweet_payload).replace("</", "<\\/")
    )
    post_script = post_script.replace(
        "SUMMARIES_PAYLOAD_PLACEHOLDER",
        json.dumps(summary_map).replace("</", "<\\/")
    )
    post_script = post_script.replace(
    "LABELS_PAYLOAD_PLACEHOLDER",
    json.dumps(label_map).replace("</", "<\\/")
)
    
    post_script = post_script.replace(
    "WINDOWS_PAYLOAD_PLACEHOLDER",
    json.dumps(windows).replace("</", "<\\/")
    )

    print("Building figure...")
    fig = build_html_with_click(clusters, windows)
    print("example tweet key:", next(iter(tweet_payload.keys())))
    print("example summary key:", next(iter(summary_map.keys())))
